...
function Projects() {
  ...
  const openPrevImage = () => {
    if (project?.src) {
      const currentIndex = selectedImageIndex ?? 0;
      setSelectedImageIndex((currentIndex - 1 + project.src.length) % project.src.length);
    }
  };
    ...
    <div className='max-w-3xl flex-1 text-xs'>
      ...
      {project.livePreview && (
        <div className='flex gap-4'>
          <a className='font-bold flex items-center mb-2 hover:underline' href={project.livePreview} target='_blank' rel="noopener noreferrer">Live Preview <MdOpenInNew className='ml-2'/></a>
          <a className='font-bold flex items-center mb-2 hover:underline' href={project.code} target='_blank' rel="noopener noreferrer">Code <MdOpenInNew className='ml-2'/></a>
        </div>
      )}
      {!project.livePreview && (
        <a className='font-bold flex items-center mb-2 hover:underline' href={project.code} target='_blank' rel="noopener noreferrer">Code</a>
      )}
